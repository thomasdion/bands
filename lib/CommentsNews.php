...
class CommentsNews {
      ...
      function countDataType($arg) {        
          
          try {
             $stmt = $this->connection->getDBH()->prepare('SELECT COUNT(*) as num FROM comments_news WHERE new=?');
             ...
             if($stmt->execute()) {                
                ...
                throw new PDOException($e,"SELECT ERROR/EXTRACT COMMENT NEWS");
           }          
      }      
    ...
    function extract($id){
          ...
          try {
                ...
                if($stmt->execute()) {   
                    $this->new = $stmt->fetch(PDO::FETCH_ASSOC);
                    $stmt->closeCursor();                    
                    $stmt2 = $this->connection->getDBH()->prepare("(SELECT c_n.id, c_n.post_date, c_n.comment,c_n.reply,  c_n.id as id2, users.username, flags.flag 
                        FROM comments_news as c_n, flags, users WHERE reply=0
                        AND c_n.user=users.user_id AND c_n.flag=flags.flag_id AND c_n.new=?)
                    ...
                    if ($stmt2->execute()) {
                        $this->comm = $stmt2->fetchAll(PDO::FETCH_ASSOC);
                    }
                 }                 
          }catch(PDOException $e){
                throw new PDOException($e,"SELECT ERROR/EXTRACT Comments");
