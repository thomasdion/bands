...
//Contents of the Accept-Charset: header from the current request, if there is one. Example: 'iso-8859-1,*,utf-8'.              
       ...
       $_SESSION['_TOKEN_ID'] = $this->tokenId;
    ...
    function validate($con){
       ...
       if(!isset($_SESSION[self::_SESSION_CONFIRMATION_])||(isset($_SESSION[self::_SESSION_CONFIRMATION_])&&!$_SESSION[self::_SESSION_CONFIRMATION_]))
           return FALSE;
       if((!isset($_SESSION['_USER_LOOSE_IP'])||(isset($_SESSION['_USER_LOOSE_IP'])&&$_SESSION['_USER_LOOSE_IP'] != long2ip(ip2long($_SERVER['REMOTE_ADDR'])&ip2long("255.255.0.0"))))||
          (!isset($_SESSION['_USER_AGENT'])||(isset($_SESSION['_USER_AGENT'])&&$_SESSION['_USER_AGENT'] != $_SERVER['HTTP_USER_AGENT']))||
          (!isset($_SESSION['_USER_ACCEPT'])||(isset($_SESSION['_USER_ACCEPT'])&&$_SESSION['_USER_ACCEPT'] != $_SERVER['HTTP_ACCEPT']))||
          (!isset($_SESSION['_USER_ACCEPT_ENC'])||(isset($_SESSION['_USER_ACCEPT_ENC'])&&$_SESSION['_USER_ACCEPT_ENC'] != $_SERVER['HTTP_ACCEPT_ENCODING']))||       
          (!isset($_SESSION['_USER_ACCEPT_LANG'])||(isset($_SESSION['_USER_ACCEPT_LANG'])&&$_SESSION['_USER_ACCEPT_LANG'] != $_SERVER['HTTP_ACCEPT_LANGUAGE']))/*||
          (!isset($_SESSION['_USER_ACCEPT_CHAR'])||(isset($_SESSION['_USER_ACCEPT_CHAR'])&&$_SESSION['_USER_ACCEPT_CHAR'] != $_SERVER['HTTP_ACCEPT_CHARSET']))*/)
            return FALSE;
       if(isset($_SESSION['_USER_ID'])&&$this->getTokenId($con, $_SESSION['_USER_ID'])) {           //Get tokenId from db
          if(isset($_SESSION['_USER_TOKEN'])&&($_SESSION['_USER_TOKEN'] != $this->tokenId)) //and see if matches with session
              return FALSE;
       }else return FALSE;     
       $current_time = time();
       if(isset($_SESSION['_KEEP_ME_LOGGED_'])&&$_SESSION['_KEEP_ME_LOGGED_']==TRUE) {
       ...
       if(isset($_SESSION['SESSION_START_TIME'])&&($current_time - $_SESSION['SESSION_START_TIME'] > $loggedTime)) //check the time that user is logged
           return FALSE; 
